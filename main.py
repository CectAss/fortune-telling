...
day = datetime.now().day                                                                            # День который помнит бот
...
def distribution(message):                                                                          # Создание функции distribution
    global day                                                                                          # Получение доступа к переменной извне для дальнейшего изменения
    if(message.text==GREETING_BUTTONS[0] or message.text==FORTUNE_TELLING_BUTTONS[0]):                  # Если пользователь ввел любой запрос про гадание (GREETING_BUTTONS[0], FORTUNE_TELLING_BUTTONS[0])
        if(datetime.now().day!=day):                                                                        # Если день, который помнит бот не совпадает с сегодняшним днем
            user_limit.clear()                                                                                  # Очистка списка (обновление дневного лимита использования гадания) 
            day = datetime.now().day                                                                            # Замена даты, которую помнит бот на сегодняшню
    ...
    elif(message.text==GREETING_BUTTONS[1]):                                                            # Если пользователь ввел запрос про вопрос (GREETING_BUTTONS[1])
    ...
    elif(message.text==GREETING_BUTTONS[2]):                                                            # Если пользователь ввел запрос про рассылку (GREETING_BUTTONS[2])
        if(str(message.chat.id) in mailing_users):                                                          # Если пользователь уже подписан на рассылку
            bot.send_message(message.chat.id, MAILING_TEXT, 
    ...
    elif(message.text==GREETING_BUTTONS[3]):                                                            # Если пользователь ввел запрос про информацию
        ...
        bot.send_message(message.chat.id, GREETING_TEXT,        
                         reply_markup=keyboard(GREETING_BUTTONS))                                           # Отправка сообщения заново
        bot.register_next_step_handler(message, distribution)                                               # Редирект в эту же функцию
...
def info(message):                                                                                  # Создание функции info                                                                            
    if(message.text==DISTRIBUTION_BUTTONS[0]):                                                          # Если пользователь нажал кнопку возврата в меню
        bot.send_message(message.chat.id, GREETING_TEXT, 
                         reply_markup=keyboard(GREETING_BUTTONS))                                           # Отправка приветствия с кнопками
